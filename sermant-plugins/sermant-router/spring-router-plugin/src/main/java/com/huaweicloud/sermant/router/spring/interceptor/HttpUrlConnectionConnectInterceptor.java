...
public class HttpUrlConnectionConnectInterceptor extends AbstractInterceptor {
    ...
    public ExecuteContext before(ExecuteContext context) {
        if (context.getObject() instanceof HttpURLConnection) {
            ...
            String sw8Correlation = connection.getRequestProperty("sw8-correlation");
            if (StringUtils.isBlank(sw8Correlation)) {
                return context;
            }
            Map<String, List<String>> tags = FlowContextUtils.decodeTags(sw8Correlation);
            if (!tags.isEmpty()) {
